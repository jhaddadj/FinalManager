...
public class LecturarRegistrationActivity extends AppCompatActivity {
                    ...
                    if (task.isSuccessful()) {
                        ...
                        uploadImageToStorage("id_images/" + userId, idImageUri, idImageUrl -> {
                            uploadImageToStorage("contract_images/" + userId, contractImageUri, contractImageUrl -> {
                                saveUserToDatabase(userId,name, email,contact, idImageUrl.toString(), contractImageUrl.toString(), userId);
                            });
                        });
                    } else {
    ...
    private void uploadImageToStorage(String path, Uri imageUri, LecturarRegistrationActivity.OnImageUploadedCallback callback) {
    ...
    private void saveUserToDatabase(String id, String name, String email, String contact, String idImageUrl, String contractImageUrl, String userId) {
        // Default preferences
                    ...
                    if (task.isSuccessful()) {
                        ...
                        preferencesRef.setValue(preferences).addOnCompleteListener(prefTask -> {
                            if (prefTask.isSuccessful()) {
                                Toast.makeText(LecturarRegistrationActivity.this, "Account Created Successfully with Preferences", Toast.LENGTH_SHORT).show();
